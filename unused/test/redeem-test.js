...
describe("RedeemAccess", function () {
  ...
  it("Expect a revert when a non-owner calls `setNodeAddress`", async function () {
    await expectRevert(
      redeem.connect(acc1).setNodeAddress("0x0000000000000000000000000000000000000000"),
  ...
  it("Expect a revert when a non-holder calls redeem", async function () {
    ...
    await mocksapphire.connect(acc1).setApprovalForAll(redeem.address, true);
    await expectRevert(
      redeem.connect(acc1).redeem(),
      "VM Exception while processing transaction: reverted with reason string 'You are not in possesion of any presale nodes",
    );
  });
});
