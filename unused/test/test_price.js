...
describe("Price Testing", function () {
  let defoOwner,
    ...
    defoTtAddress,
    factoryAddress,
    swapTokensToLiquidityThreshold;
  let defo, lpManager;
  let table = new Table({
  ...
  beforeEach(async function () {
    ...
    lpManager = await LpManager.connect(lpManagerOwner).deploy(
      routerAddress,
  ...
  it("Testing Price based on reserve", async function () {
    ...
    await routerContract
      .connect(defoOwner)
      .swapExactTokensForTokens(amountToSwap, "0", [dai.address, defo.address], defoOwner.address, "1649910829447");
    ...
    await routerContract
      .connect(defoOwner)
      .swapExactTokensForTokens(sellingAmount, "0", [defo.address, dai.address], defoOwner.address, "1649910829447");
