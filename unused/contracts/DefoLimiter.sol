...
contract DefoLimiter is AccessControlUpgradeable, OwnableUpgradeable {
    ...
    function initialize(address _defoNodeAddress, address _diamondAddress) public initializer {
    ...
    modifier onlyDefoToken() {
        require(address(msg.sender) == address(DefoToken), "Only Defo ERC-20 contract can call this function");
        _;
    }
    ...
    modifier notDenied(address from, address to) {
        require(!Blocklist[from] && !Blocklist[to], "Address is not permitted");
        _;
    }

    event UserLimiterBuy(address indexed _from, address indexed _to);

    event UserLimiterSell(address indexed _from, address indexed _to);

    function isPair(address _checkAddress) private view returns (bool) {
    ...
    function transferLog(
        ...
        address to,
        uint256 amount
    ) external onlyDefoToken checkTimeframe notDenied(from, to) returns (bool) {
    ...
    function editWhitelist(address _address, bool _allow) external onlyRole(DEFAULT_ADMIN_ROLE) {
        Whitelist[_address] = _allow;
    }

    function editBlocklist(address _address, bool _allow) external onlyRole(DEFAULT_ADMIN_ROLE) {
