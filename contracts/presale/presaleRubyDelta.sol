...
// File: contracts/RubyNodeDelta.sol
...
/// @title Defo Ruby Delta Node Minting Contract
/// @notice Contract for minting ruby nodes
contract RubyNodeDelta is ERC721Enumerable, Ownable {
    using SafeMath for uint256;
    using Counters for Counters.Counter;
    Counters.Counter private supply;

    bool public activeSale = false;
    uint64 public maxTotalRubyNodes = 120; // total supply of ruby nodes
    uint64 public maxNodesPerWallet = 5; // max number of ruby nodes one can hold per wallet
    uint256 public rubyNodePrice = 300; // the current price for minting a ruby node
    ...
    constructor(address DAI_address) ERC721("Defo Ruby Delta Node", "DRDN") {
        ...
        /*for (uint256 index = 0; index < 22; index++) {
            supply.increment();
    ...
    function mintNode(uint256 amount) public {
        ...
        require(
            supply.current() + amount < maxTotalRubyNodes,
            "No more ruby nodes to mint"
        );
        require(
            DAIToken.balanceOf(msg.sender) >=
            (rubyNodePrice * DECIMAL_MULTIPLIER) * amount,
    ...
    function giveAwayMint() public onlyOwner {
        require(
            supply.current() < maxTotalRubyNodes,
            "No more ruby nodes to mint"
        );
        supply.increment();
        _safeMint(msg.sender, supply.current());
