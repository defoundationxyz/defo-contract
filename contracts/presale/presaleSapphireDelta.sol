...
// File: contracts/SapphireNodeDelta.sol
...
/// @title Defo Sapphire Delta Node Minting Contract
/// @notice Contract for minting sapphire nodes

contract SapphireNodeDelta is ERC721Enumerable, Ownable {
    using SafeMath for uint256;
    using Counters for Counters.Counter;
    Counters.Counter private supply;

    bool public activeSale = false;
    uint64 public maxTotalSapphireNodes = 240; // total supply of sapphire nodes
    uint64 public maxNodesPerWallet = 10; // max number of sapphire nodes one can hold per wallet
    uint256 public sapphireNodePrice = 75;
    ...
    ERC721("Defo Sapphire Delta Node", "DSDN")
    {
        ...
        /*for (uint256 index = 0; index < 41; index++) {
            supply.increment();
            _safeMint(FEE_COLLECTOR, supply.current());
        }*/
    }
    ...
    function mintNode(uint256 amount) public {
        ...
        require(
            balanceOf(msg.sender) + amount <= maxNodesPerWallet,
            "Cannot mint more than `maxNodePerWallet` nodes"
        );
        require(
            supply.current() + amount < maxTotalSapphireNodes,
            "No more sapphire nodes to mint"
        );
        require(
            DAIToken.balanceOf(msg.sender) >=
            (sapphireNodePrice * DECIMAL_MULTIPLIER) * amount,
    ...
    function giveAwayMint() public onlyOwner {
        require(
            supply.current() < maxTotalSapphireNodes,
            "No more sapphire nodes to mint"
        );
        supply.increment();
        _safeMint(msg.sender, supply.current());
    ...
    function setMaxTotalSapphireNodes(uint64 _newNodeAmount)
    external
    onlyOwner
    {
