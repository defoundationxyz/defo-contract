...
// File: contracts/DiamondNodeDelta.sol
...
/// @title Defo Diamond Delta Node Minting Contract
/// @notice Contract for minting diamond nodes
contract DiamondNodeDelta is ERC721Enumerable, Ownable {
    using SafeMath for uint256;
    using Counters for Counters.Counter;
    Counters.Counter private supply;

    bool public activeSale = false;
    uint64 public maxTotalDiamondNodes = 60; // total supply of diamond nodes
    uint64 public maxNodesPerWallet = 1; // max number of diamond nodes one can hold per wallet
    uint256 public diamondNodePrice = 1200; // the current price for minting a diamond node
    ...
    constructor(address DAI_address) ERC721("Defo Diamond Delta Node", "DDDN") {
    ...
    function mintNode(uint256 amount) public {
        ...
        require(
            supply.current() + amount < maxTotalDiamondNodes,
            "No more diamond nodes to mint"
        );
        require(
            DAIToken.balanceOf(msg.sender) >=
            (diamondNodePrice * DECIMAL_MULTIPLIER) * amount,
