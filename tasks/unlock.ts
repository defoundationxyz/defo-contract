...
export default task("unlock", "releases the contracts to teh normal operation mode").setAction(async (_, hre) => {
  const { ethers } = hre;
  ...
  await (await defoContract.unpause()).wait();
  await (await defoDiamond.unpause()).wait();
  await (await defoDiamond.unlockMint()).wait();
