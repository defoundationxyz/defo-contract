...
import {FUJI_DEFO_ADDRESS} from '@constants/addresses';
...
describe('DEFO Token', async () => {
  ...
  before(async () => {
    if (await isLocalForkingMainnetEnvironment(hardhat)) {
      ...
      defoContract = await getContractWithSigner<DEFOToken>(hardhat, 'DEFOToken');
    } else {
      defoContract ??= await getContractAt<DEFOToken>('DEFOToken', FUJI_DEFO_ADDRESS);
    }
  });
